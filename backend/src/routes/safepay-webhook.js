...
export async function handleSafePayWebhook(req,res,{provider}){
  ...
  const existing=get('SELECT id,processed FROM safepay_events WHERE provider=? AND event_id=?',[provider,eventId]);
  if(existing) return res.json(200,{ok:true,idempotent:true});
  const order=get('SELECT * FROM market_orders WHERE provider=? AND provider_escrow_id=?',[provider,escrowId]);
  const inserted=run('INSERT INTO safepay_events (provider,event_id,order_id,event_type,payload_json,processed) VALUES (?,?,?,?,?,0)',
  ...
  run("UPDATE market_orders SET status='FUNDED',updated_at=datetime('now') WHERE id=?",[order.id]);
  run('UPDATE safepay_events SET processed=1 WHERE id=?',[inserted.lastInsertRowid]);
