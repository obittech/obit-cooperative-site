...
function anchorCommunity() {
  const community = get('SELECT * FROM communities WHERE slug = ?', [ANCHOR_COMMUNITY_SLUG]);
...
safePayRouter.get('/api/market/listings', async (req, res) => {
  const community = anchorCommunity();
  const rows = all("SELECT id,title,description,price_kobo,currency,category,status,created_at FROM market_listings WHERE community_id=? AND status='ACTIVE' ORDER BY id DESC LIMIT 100", [community.id]);
...
safePayRouter.post('/api/market/listings', requireAuth('member'), async (req, res) => {
  ...
  const result = run(`INSERT INTO market_listings (community_id,seller_member_id,title,description,category,price_kobo,currency,status)
...
safePayRouter.post('/api/market/orders', requireAuth('member'), async (req, res) => {
  ...
  const community = anchorCommunity();
  const listing = get("SELECT * FROM market_listings WHERE id=? AND community_id=? AND status='ACTIVE'", [Number(req.body?.listing_id), community.id]);
